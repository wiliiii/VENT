<?php
	...
	class clientController extends mainModel{
		...
		public function registrarClienteControlador(){
			...
			# Verificando nombre, apellido, provincia, ciudad y dirección #
			if($this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{3,40}", $nombre) || $this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{3,40}", $apellido)){
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "El NOMBRE o APELLIDO no coincide con el formato solicitado",
					"icono" => "error"
				];
				return json_encode($alerta);
				exit();
			...
			if($this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{4,30}", $provincia) || $this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{4,30}", $ciudad)){
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "La PROVINCIA o CIUDAD no coincide con el formato solicitado",
					"icono" => "error"
				];
				return json_encode($alerta);
				exit();
			...
			if($check_documento->rowCount() > 0){
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "El número y tipo de documento ya se encuentran registrados",
					"icono" => "error"
				];
				return json_encode($alerta);
				exit();
		...
		public function listarClienteControlador($pagina, $registros, $url, $busqueda){
			...
			if(isset($busqueda) && $busqueda != ""){
				...
				$consulta_datos = "SELECT * FROM cliente WHERE cliente_id != 1 ORDER BY cliente_nombre ASC LIMIT $registros OFFSET $inicio";
		...
		public function eliminarClienteControlador(){
			...
			if($check_ventas->rowCount() > 0){
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "No podemos eliminar el cliente ya que tiene ventas asociadas",
					"icono" => "error"
				];
				return json_encode($alerta);
				exit();
			...
			if($eliminarCliente->rowCount() == 1){
				...
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "No hemos podido eliminar el cliente ".$datos['cliente_nombre']." ".$datos['cliente_apellido']." del sistema",
					"icono" => "error"
				];
			}
