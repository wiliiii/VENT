	...
	class cashierController extends mainModel{
		...
		public function listarCajaControlador($pagina, $registros, $url, $busqueda){
			...
			if(isset($busqueda) && $busqueda != ""){
				$consulta_datos = "SELECT * FROM caja WHERE caja_numero LIKE '%$busqueda%' OR caja_nombre LIKE '%$busqueda%' ORDER BY caja_numero ASC LIMIT $registros OFFSET $inicio";
				$consulta_total = "SELECT COUNT(caja_id) FROM caja WHERE caja_numero LIKE '%$busqueda%' OR caja_nombre LIKE '%$busqueda%'";
			} else {
				$consulta_datos = "SELECT * FROM caja ORDER BY caja_numero ASC LIMIT $registros OFFSET $inicio";
