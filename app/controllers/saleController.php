<?php
	...
	class saleController extends mainModel{
		...
		public function buscarCodigoVentaControlador(){
			...
			/*== Comprobando que no esté vacío el campo ==*/
			if ($producto == "") {
				return '
			...
			$consulta = "SELECT * FROM producto WHERE (producto_nombre LIKE :producto OR producto_marca LIKE :producto OR producto_modelo LIKE :producto) ORDER BY producto_nombre ASC";
			$datos_productos = $this->ejecutarConsulta($consulta, ['producto' => "%$producto%"]);
		...
		public function agregarProductoCarritoControlador() {
			...
			$consulta = "SELECT * FROM producto WHERE producto_codigo = :codigo";
			$check_producto = $this->ejecutarConsulta($consulta, ['codigo' => $codigo]);
		...
		public function listarVentaControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE (venta.venta_codigo='$busqueda') ORDER BY venta.venta_id DESC LIMIT $registros OFFSET $inicio";

				$consulta_total="SELECT COUNT(venta_id) FROM venta WHERE (venta.venta_codigo='$busqueda')";

			}else{

				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id ORDER BY venta.venta_id DESC LIMIT $registros OFFSET $inicio";
